    ...
    if (this.cargoBooking?.id != null) {
      ...
      this.bookingSerice.getPackageAuditStatus(this.cargoBooking.id).subscribe(
        (res:any) => {
          ...
          this.offloaded = res.filter((x:PackageModel)=> x.packageItemStatus == PackageItemStatus.Offloaded).fliter((x:PackageModel) => this.uld_packed.filter((y)=> y.packageID == x.packageID).length==0)
